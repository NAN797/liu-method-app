...
with col1:
    energy_input = st.text_area(trans("输入脉冲能量 E (μJ)", "Input pulse energy E (μJ)"), def_energy)
with col2:
    diameter_input = st.text_area(trans("输入烧蚀直径 D (μm)", "Input ablation diameter D (μm)"), def_diameter)

uploaded_file = st.file_uploader(trans("或上传 CSV 文件（两列: E, D）", "Or upload a CSV file (two columns: E, D)"))

if st.button(trans("开始拟合", "Fit Now")):
    try:
        if uploaded_file:
        ...
        if len(E_values) != len(D_values):
            st.error(trans("能量和直径数量不一致！", "Mismatch in number of values between energy and diameter!"))
        else:
            ...
            st.success(trans("拟合成功 ✅", "Fit successful ✅"))
            st.markdown(f"**{trans('阈值能量 E_th', 'Threshold energy E_th')}** ≈ `{E_th:.2f}` μJ")
            st.markdown(f"**{trans('光斑半径 w₀', 'Beam waist w₀')}** ≈ `{w0:.2f}` μm")
            st.markdown(f"**{trans('阈值通量 F_th', 'Fluence threshold F_th')}** ≈ `{F_th:.5f}` J/cm²")
            st.markdown(f"**R² 值 / R-squared** ≈ `{r_value**2:.4f}`")

            fig, ax = plt.subplots()
            ln_E_fit = np.linspace(ln_E.min(), ln_E.max(), 100)
            D_fit = slope * ln_E_fit + intercept
            ax.scatter(ln_E, D_squared, color='blue', label=trans('实验数据', 'Experimental Data'))
            ax.plot(ln_E_fit, D_fit, color='red', label=trans('线性拟合', 'Linear Fit'))
            ax.set_xlabel('ln(E)')
            ax.set_ylabel('D² (μm²)')
            ax.set_title(trans('Liu 方法拟合图', 'Liu Method Fit'))
            ax.legend()
            ax.grid(True)
            st.pyplot(fig)
            ...
            with pd.ExcelWriter(towrite, engine='xlsxwriter') as writer:
                ...
                writer.save()
            towrite.seek(0)
            st.download_button(trans("📥 下载 Excel", "📥 Download Excel"), towrite, file_name="liu_fit_results.xlsx")

    except Exception as e:
        st.error(f"{trans('发生错误：', 'Error occurred: ')}{e}")
